...
only_export_these_tags = ['won', 'shared']  # Leave this list empty for all notes! See below for sample
...
import os
HOME = os.getenv('HOME', '')
default_out_folder = os.path.join(HOME, "Workspace", "WonSong", "temp")
default_backup_folder = os.path.join(HOME, "Workspace", "WonSong", "backup")
...
parser.add_argument("--images", default="", help="Path where images will be stored")
...
parser.add_argument("--excludeTag", action="append", default=['journal', 'apeak', 'permanent'], help="Don't export notes with this tag. Can be used multiple times.")
...
assets_path = parsed_args.get("images") if parsed_args.get("images") else os.path.join(export_path, 'assets')
